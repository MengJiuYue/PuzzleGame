...
public class GameJFrame extends JFrame implements KeyListener, ActionListener {
    final static String DB_URL = "jdbc:h2:./data/game";
    ...
    private void initializeGameData() {
        ...
        /* mysql游戏数据 */
        JLabel avgStep = new JLabel("场均步数"+gameData[0]);
        avgStep.setBounds(380, 30, 100, 20);
        this.add(avgStep);

        JLabel count = new JLabel("完成局数"+gameData[1]);
        count.setBounds(100, 30, 100, 20);
        this.add(count);

        JLabel stepRank = new JLabel("均步排名"+getRank());
    ...
    private void changeImg(String kind) {
        ...
        // STR."/image/\{kind}/\{kind}\{Index}/"
        PATH =PATH = "/image/" + kind + "/" + kind + Index + "/";
        step = 0;
        imgIndex = initializeImgIndex();
        loadGameData(gameData, nowAccount);
        initializeGameData();
